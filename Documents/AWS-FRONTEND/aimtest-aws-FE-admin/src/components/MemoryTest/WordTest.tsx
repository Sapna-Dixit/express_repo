...
import {
  ...
  postWordTestRange,
  putWordTestRange,
} from 'redux/action/Test/WordTest';
...
import { getWordTestLevels } from 'redux/action/Level/WordTest';
...
const WordTest = () => {
  ...
  const testData = useSelector(
    (state: RootState) => state?.wordTest?.wordTestData?.wordTests
  );
  const userRole = useSelector((state: RootState) => state?.login?.userRole);

  const load = useSelector((state: RootState) => state?.loader.loader);
  const numberOfRecord = useSelector(
    (state: RootState) => state?.wordTest?.wordTestData?.numberOfRecord
  );
  ...
  const wordTestLevel = useSelector(
    (state: RootState) => state?.wordTestLevel?.wordTestLevelData?.wordTests
  );
  ...
  const initialValues: wordTestRange = {
    wordTestDetail:
      todo === 'edit'
        ? editData
        : [
  ...
  const levelListOptions = (digit: number, index: number) => {
    const filteredList = wordTestLevel?.filter(
            ...
            <Formik
              ...
              onSubmit={(values, { resetForm }) => {
                if (todo === 'edit') {
                  ...
                  dispatch(
                    putWordTestRange(
                      values.wordTestDetail[0]._id as string,
                      values.wordTestDetail[0] as rangeData,
                      token,
                      setDisplay,
                      resetForm
            ...
            >
              {({
                ...
                <>
                  {' '}
                  <RestoreLevelModal
                    title='Restore Word Test'
                    modalName='Word Test'
                    restoreId={restore}
                    resetForm={resetForm}
                    navigate={setDisplay}
                    content='Word Test Exist But Soft Deleted. Do You Want To Restore The Data'
                    show={restoreLevel}
                                      ...
                                      <div key={index}>
                                        <div className='row'>
                                          <div className='col'>
                                            ...
                                            <SelectField
                                              name={`wordTestDetail[${index}].class.name`}
                                              options={classList(dataClass)}
                                              placeholder='Select Class'
                                              value={
                                                values?.wordTestDetail[index]
                                                  ?.class?.name
                                              }
                                              onChange={(e) =>
                                                setFieldValue(
                                                  `wordTestDetail[${index}].class`,
                                                  e &&
                                              ...
                                              touched={
                                                touched.wordTestDetail !==
                                                  undefined &&
                                                touched?.wordTestDetail[index]
                                                  ?.class?.name
                                              }
                                            />
                                          </div>
                                          <div className='col'>
                                            ...
                                            <div className='row'>
                                              <div className='col pe-0'>
                                                <SelectField
                                                  name={`wordTestDetail[${index}].from.level`}
                                                  isSearchable={false}
                                                  options={wordTestLevel
                                                    ?.map(
                                                      (item: {
                                                        level: number;
                                                        _id: string;
                                                  ...
                                                  value={
                                                    values?.wordTestDetail[
                                                      index
                                                    ]?.from?.level as string
                                                  }
                                                  onChange={(e) => {
                                                    setFieldValue(
                                                      `wordTestDetail[${index}].to`,
                                                      {
                                                        level: '',
                                                        _id: '',
                                                      }
                                          ...
                                          <div className='col'>
                                            ...
                                            <div className='row'>
                                              <div className='col pe-0'>
                                                <SelectField
                                                  name={`wordTestDetail[${index}].to.level`}
                                                  isSearchable={false}
                                ...
                                <div>
                                  <div className={cx('card-flex')}>
                                    <ol className='cd-breadcrumb'>
                                      ...
                                      <li className='current'>Word Test</li>
                                    </ol>
                                  </div>
