...
import { getSubFranchisees } from 'redux/action/SubFranchisee';
import SubFranchiseeTable from 'components/Table/SubFranchiseeTable';
import AddSubFranchisee from 'components/AddSubFranchisee/AddSubFranchisee';
...
const SubFranchisee = () => {
  ...
  const subFranchisee = useSelector(
    (state: RootState) =>
  ...
  useEffect(() => {
    const dataToDisplay = subFranchisee?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, subFranchisee]);
  ...
  useEffect(() => {
    if (
      ...
      dispatch(loader(true)),
        dispatch(getSubFranchisees(adminToken) as unknown as AnyAction);
    }
  ...
  return (
    <>
      {addForm === false ? (
        ...
        <AddSubFranchisee setAddSubFranchisee={setAddForm} />
      )}
    </>
  );
};
