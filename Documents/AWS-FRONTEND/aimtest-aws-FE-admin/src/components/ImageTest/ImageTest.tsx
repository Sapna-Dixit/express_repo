...
import { getImageTests } from 'redux/action/Test/ImageTest';
import ImageTestTable from 'components/Table/ImageTestTable';
import AddImageTest from 'components/AddTest/ImageTest/ImageTest';
  ...
  useEffect(() => {
    if (adminToken?.length > 0 && query?.editId === undefined) {
      dispatch(loader(true)),
        dispatch(getImageTests(adminToken) as unknown as AnyAction);
    }
  ...
  return (
    <>
      {addForm === false ? (
        ...
        <AddImageTest setAddImageTest={setAddForm} />
      )}
    </>
  );
};
