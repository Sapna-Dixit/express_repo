...
import InterestTable from 'components/Table/InterestTest';
import { getInterestTests } from 'redux/action/Test/Interest';
import AddInterestTest from 'components/AddTest/InterestTest/InterestTest';
  ...
  useEffect(() => {
    if (adminToken?.length > 0 && query?.editId === undefined) {
      dispatch(loader(true)),
        dispatch(getInterestTests(adminToken) as unknown as AnyAction);
    }
  ...
  return (
    <>
      {addForm === false ? (
        ...
        <AddInterestTest setAddInterestTest={setAddForm} />
      )}
    </>
  );
};
