...
import { getInterestTest } from 'redux/action/Test/Interest';

const InterestTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const interestTestTodo = Cookies.get('delete');

  const interestTestData = useSelector(
    (state: RootState) => state?.interestTest?.interestTestData?.interestTests
  );
  ...
  useEffect(() => {
    const dataToDisplay = interestTestData?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, interestTestData]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, interestTestTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = interestTestData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: interestTestData) => {
        return Object?.keys(item)?.some(
          () =>
            item &&
            ((item &&
              item?.interest?.some((item1) => {
                return item1
                  ?.toString()
                  ?.toLowerCase()
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: interestTestData) =>
  ...
  const apiCalling = (id: string) => {
    dispatch(getInterestTest(id, adminToken) as unknown as AnyAction);
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.replace(`/test/interest?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: interestTestData, index: number) => {
                  return (
                    <>
                      <tr key={item._id} className='table-data'>
                        ...
                        <td>
                          {item?.interest?.map((v, i) =>
                            i === item?.interest?.length - 1 ? v : `${v} | `
                          )}
                        </td>
