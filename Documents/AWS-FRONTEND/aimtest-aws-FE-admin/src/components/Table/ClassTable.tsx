...
import { getClass } from 'redux/action/Class';
import ClassModal from 'components/Modal/ClassModal';
import { classList } from 'components/MemoryTest/GetClassList';

const ClassTable = (props: tablePropsType) => {
  const dispatch = useDispatch();
  const classPageTodo = Cookies.get('delete');
  ...
  useEffect(() => {
    const list = classList(classData);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, classPageTodo !== undefined]);

  const getSearch = () => {
    let list = classData;
    if (props?.searchData?.length > 0) {
      list = list?.filter((item: classDataType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: classDataType) =>
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        dispatch(getClass(id, adminToken) as unknown as AnyAction))
      : selected?.length > 1
          ...
          <div className='table-scroll-height'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: classDataType, index: number) => {
                  return (
                    <>
