...
import { getPassage } from 'redux/action/Test/Passage';

const PassageTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const passageTodo = Cookies.get('delete');

  const passageData = useSelector(
    (state: RootState) => state?.passage?.passageData?.passageTests
  );
  ...
  useEffect(() => {
    const dataToDisplay = passageData?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, passageData]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, passageTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = passageData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: passageDataType) => {
        return Object?.keys(item)?.some(
          () =>
            item &&
            ((item.passageDisplayTime &&
              item.passageDisplayTime
                ?.toString()
                ?.toLowerCase()
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: passageDataType) =>
  ...
  const apiCalling = (id: string) => {
    dispatch(getPassage(id, adminToken) as unknown as AnyAction);
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.replace(`/test/passage?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              <tr className='table-heading'>
                ...
                <td>
                  <div>Passage Display Time</div>
                </td>
                <td>Action</td>
              </tr>
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: passageDataType, index: number) => {
                  return (
                    <>
                      <tr key={item._id} className='table-data'>
                        ...
                        <td>{item?.passageDisplayTime} Min</td>
                        <td>
