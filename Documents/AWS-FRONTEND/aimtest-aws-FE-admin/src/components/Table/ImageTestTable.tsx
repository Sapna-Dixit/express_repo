...
import { getImageTest } from 'redux/action/Test/ImageTest';
import { getImageTestLevels } from 'redux/action/Level/ImageTest';

const ImageTestTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const imageTestTodo = Cookies.get('delete');

  const imageTestData = useSelector(
    (state: RootState) => state?.imageTest?.imageTestData?.imageTests
  );
  ...
  useEffect(() => {
    const dataToDisplay = imageTestData?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, imageTestData]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, imageTestTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = imageTestData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: imageTestType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: imageTestType) =>
  ...
  const apiCalling = (id: string) => {
    dispatch(getImageTest(id, adminToken) as unknown as AnyAction);
    dispatch(getClasses(adminToken) as unknown as AnyAction);
    dispatch(getImageTestLevels(adminToken) as unknown as AnyAction);
  };
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.replace(`/test/imagetest?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: imageTestType, index: number) => {
                  return (
                    <>
