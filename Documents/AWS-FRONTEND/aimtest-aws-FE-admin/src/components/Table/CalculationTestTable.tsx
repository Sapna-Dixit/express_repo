...
import { getCalculationTest } from 'redux/action/Test/CalculationTest';
import { getCalculationTestLevels } from 'redux/action/Level/CalculationTest';

const CalculationTestTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const calculationTestTodo = Cookies.get('delete');
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, calculationTestTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = calculationTestData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: calculationTestType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: calculationTestType) =>
  ...
  const apiCalling = (id: string) => {
    dispatch(getCalculationTest(id, adminToken) as unknown as AnyAction);
    dispatch(getClasses(adminToken) as unknown as AnyAction);
    dispatch(getCalculationTestLevels(adminToken) as unknown as AnyAction);
  };
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.push(`/test/calculationtest?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: calculationTestType, index: number) => {
                  return (
                    <>
...
export default CalculationTestTable;
