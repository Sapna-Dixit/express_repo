...
const WritingTestTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const writingTestTodo = Cookies.get('delete');

  const writingTestData = useSelector(
    (state: RootState) => state?.writingTest?.writingTestData?.writingTests
  );
  ...
  useEffect(() => {
    const dataToDisplay = writingTestData?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, writingTestData]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, writingTestTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = writingTestData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: writingTestType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: writingTestType) =>
  ...
  const Edit = (id: string) => {
    localStorage.setItem('select', '/add/writingtest');
    setTodo('edit');
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.replace(`/test/writingtest?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: writingTestType, index: number) => {
                  return (
                    <>
