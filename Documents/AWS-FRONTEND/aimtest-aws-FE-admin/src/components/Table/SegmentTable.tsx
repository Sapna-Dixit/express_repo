...
import { getSegment } from 'redux/action/Segment';
import SegmentModal from 'components/Modal/SegmentModal';

const SegmentTable = (props: tablePropsType) => {
  const dispatch = useDispatch();
  const segmentPageTodo = Cookies.get('delete');
  ...
  useEffect(() => {
    const dataToDisplay = segmentData?.slice(startingIndex, endingIndex);
    setPageWiseData(dataToDisplay);
  }, [startingIndex, endingIndex, segmentData]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, segmentPageTodo !== undefined]);
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: sectionDataType) =>
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        dispatch(getSegment(id, adminToken) as unknown as AnyAction))
      : selected?.length > 1
          ...
          <div className='table-scroll-height'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: sectionDataType, index: number) => {
                  return (
                    <>
