...
import { getSubFranchisee } from 'redux/action/SubFranchisee';

let a: string[];

const SubFranchiseeTable = (props: tablePropsType) => {
  ...
  const subFranchiseeData = useSelector(
    (state: RootState) =>
      state?.subFranchisee?.subFranchiseeData?.subFranchisees
  );
  ...
  useEffect(() => {
    const dataToDisplay = subFranchiseeData?.slice(startingIndex, endingIndex);
  ...
  }, [startingIndex, endingIndex, subFranchiseeData, parentFranchisee?._id]);
  ...
  const getSearch = () => {
    let list = subFranchiseeData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: subfranchiseeType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: subfranchiseeType) =>
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        dispatch(getSubFranchisee(id, adminToken) as unknown as AnyAction),
  ...
  // const reDirectToStudent = (name: string, _id: string) => {
  //   router.push('/school/franchisee'),
          ...
          <div className='table-scroll-subfranchisee'>
            <table className='customers'>
              <tr className='table-heading'>
                ...
                <td>S No.</td>
                <td>
                  <div>Name of Sub Franchisee</div>
                </td>
                <td>
                  <div>State</div>
                </td>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: subfranchiseeType, index: number) => {
                  return (
                    <>
                          ...
                          onClick={() => {
                            ...
                            localStorage.setItem(
                              ...
                              JSON.stringify({
                                _id: item?._id,
                                name: item?.name,
                                country: parentFranchisee?.country,
                              })
                            );
                            franchiseeName.push({
                              _id: item._id,
...
export default SubFranchiseeTable;
