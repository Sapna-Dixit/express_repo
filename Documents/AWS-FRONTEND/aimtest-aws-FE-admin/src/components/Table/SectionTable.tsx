...
import { getSection } from 'redux/action/Section';
import SectionModal from 'components/Modal/SectionModal';

const SectionTable = (props: tablePropsType) => {
  const dispatch = useDispatch();
  const sectionPageTodo = Cookies.get('delete');
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, sectionPageTodo !== undefined]);
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        dispatch(getSection(id, adminToken) as unknown as AnyAction))
      : selected?.length > 1
