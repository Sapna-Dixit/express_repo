...
import { getAudioVideoTest } from 'redux/action/Test/AudioVideo';

const AudioVideoTestTable = (props: tablePropsType) => {
  const router = useRouter();
  const dispatch = useDispatch();
  const audioVideoTestTodo = Cookies.get('delete');
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [props.valueOfPage, audioVideoTestTodo !== undefined]);
  ...
  const getSearch = () => {
    let list = audioVideoTestData;
    if (props.searchData?.length > 0) {
      list = list?.filter((item: audioVideoType) => {
  ...
  const globalCheck = (e: ChangeEvent<HTMLInputElement>) => {
    if (e?.target?.checked) {
      ...
      pageWiseData?.map((item: audioVideoType) =>
  ...
  const apiCalling = (id: string) => {
    dispatch(getAudioVideoTest(id, adminToken) as unknown as AnyAction);
  ...
  const Edit = (id: string) => {
    ...
    selected?.length === 1 && selected[0] === id
      ? (setSelectedType('single'),
        setId(id),
        apiCalling(id),
        router.replace(`/test/audiovideotest?editId=${id}`))
      : selected?.length > 1
          ...
          <div className='table-scroll-question'>
            <table className='customers'>
              ...
              {(props?.searchData?.length ? getSearch() : pageWiseData)?.map(
                (item: audioVideoType, index: number) => {
                  return (
                    <>
                      <tr key={item._id} className='table-data'>
                        ...
                        <td>{item?.type === 'video' ? 'Video' : 'Audio'}</td>
...
export default AudioVideoTestTable;
