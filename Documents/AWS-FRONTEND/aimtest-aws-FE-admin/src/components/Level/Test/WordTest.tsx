...
import {
  ...
  postWordTestLevel,
  putWordTestLevel,
} from 'redux/action/Level/WordTest';
...
const WordTest = ({ tabName }: { tabName: string }) => {
  ...
  const numberOfRecord = useSelector(
    (state: RootState) =>
      state?.wordTestLevel?.wordTestLevelData?.numberOfRecord
  );
  ...
  const [restoreWordLevel, setRestoreWordLevel] = useState(false);
  ...
  useEffect(() => {
    if (tabName === 'wordtest' && restore?.length > 0) {
      setRestoreWordLevel(true);
    }
  ...
  const initialValues: TestValues = {
    ...
    noOfWords:
      todo === 'edit'
        ? (editData as { noOfWords: string | number }).noOfWords
        : '',
    marks:
            ...
            <Formik
              ...
              validationSchema={
                ...
                Yup.object().shape({
                  ...
                  noOfWords: Yup.number().required('Required'),
              ...
              {({ values, handleSubmit, resetForm }) => (
                <>
                  <RestoreLevelModal
                    ...
                    show={restoreWordLevel}
                    onClose={() => setRestoreWordLevel(false)}
                  />
                  {display ? (
