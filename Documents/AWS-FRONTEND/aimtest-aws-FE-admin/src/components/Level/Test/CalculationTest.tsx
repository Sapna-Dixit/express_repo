...
import {
  getCalculationTestLevels,
  postCalculationTestLevel,
  putCalculationTestLevel,
} from 'redux/action/Level/CalculationTest';
import { loader } from 'redux/reducer/Loader';
import DeleteModal from 'components/Modal/DeleteModal';
import RestoreLevelModal from 'components/Modal/RestoreLevelModal';

const CalculationTest = ({ tabName }: { tabName: string }) => {
  ...
  const testData = useSelector(
    (state: RootState) =>
      state?.calculationTestLevel?.calculationTestLevelData
        ?.calculationTestLevels
  );

  const load = useSelector((state: RootState) => state?.loader?.loader);
  const numberOfRecord = useSelector(
    (state: RootState) =>
      state?.calculationTestLevel?.calculationTestLevelData?.numberOfRecord
  );
  ...
  const [restoreCalculationLevel, setRestoreCalculationLevel] = useState(false);
  ...
  useEffect(() => {
    if (tabName === 'calculationtest' && restore?.length > 0) {
      setRestoreCalculationLevel(true);
    }
  ...
  const initialValues: TestValues = {
    ...
    noOfOperations:
      todo === 'edit'
        ? (editData as { noOfOperations: string | number }).noOfOperations
        : '',
    marks:
  ...
  return (
    <>
      <DeleteModal
        modalName='Level Calculation Test'
        show={modalShow}
            ...
            <Formik
              ...
              validationSchema={
                ...
                Yup.object().shape({
                  ...
                  noOfOperations: Yup.number().required('Required'),
              ...
              {({ values, handleSubmit, resetForm }) => (
                <>
                  <RestoreLevelModal
                    ...
                    show={restoreCalculationLevel}
                    onClose={() => setRestoreCalculationLevel(false)}
                  />
                  {display ? (
                          ...
                          <div>
                            ...
                            <Row className={cx('mb-2')}>
                              ...
                              <Col sm={4}>
                                <h6 className={cx('label-style')}>
                                  {' '}
                                  Equation Display Duration In Sec
                                </h6>
                                <Field
                              ...
                              <div className={cx(!display && 'table-level')}>
                                <table className='customers'>
                                  <tr className='table-heading'>
                                    ...
                                    <td>
                                      <div>No of Operations</div>
                                    </td>
                                    <td>Marks</td>
                                    <td>Equation Display Duration</td>
