...
import {
  ...
  postImageTestLevel,
  putImageTestLevel,
} from 'redux/action/Level/ImageTest';
...
const ImageTest = ({ tabName }: { tabName: string }) => {
  ...
  const numberOfRecord = useSelector(
    (state: RootState) =>
      state?.imageTestLevel?.imageTestLevelData?.numberOfRecord
  );
  ...
  const [restoreImageLevel, setRestoreImageLevel] = useState(false);
  ...
  useEffect(() => {
    if (tabName === 'imagetest' && restore?.length > 0) {
      setRestoreImageLevel(true);
    }
  ...
  const initialValues: TestValues = {
    ...
    noOfImages:
      todo === 'edit'
        ? (editData as { noOfImages: string | number }).noOfImages
        : '',
    marks:
            ...
            <Formik
              ...
              validationSchema={
                ...
                Yup.object().shape({
                  ...
                  noOfImages: Yup.number().required('Required'),
              ...
              {({ values, handleSubmit, resetForm }) => (
                <>
                  <RestoreLevelModal
                    ...
                    show={restoreImageLevel}
                    onClose={() => setRestoreImageLevel(false)}
                  />
                  {display ? (
