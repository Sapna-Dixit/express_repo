...
import { getPassages } from 'redux/action/Test/Passage';
import PassageTable from 'components/Table/PassageTable';
import AddPassage from 'components/AddTest/Passage/Passage';
  ...
  useEffect(() => {
    if (adminToken?.length > 0 && query?.editId === undefined) {
      dispatch(loader(true)),
        dispatch(getPassages(adminToken) as unknown as AnyAction);
    }
  ...
  return (
    <>
      {addForm === false ? (
        ...
        <AddPassage setAddPassageTest={setAddForm} />
      )}
    </>
  );
};
