...
import { getWritingTests } from 'redux/action/Test/WritingTest';
import WritingTestTable from 'components/Table/WritingTestTable';
import AddWritingTest from 'components/AddTest/WritingTest/WritingTest';
  ...
  useEffect(() => {
    if (adminToken?.length > 0 && query?.editId === undefined) {
      dispatch(loader(true)),
        dispatch(getWritingTests(adminToken) as unknown as AnyAction);
    }
  ...
  return (
    <>
      {addForm === false ? (
        ...
        <AddWritingTest setAddWritingTest={setAddForm} />
      )}
    </>
  );
};
